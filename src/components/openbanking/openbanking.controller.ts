...
export class OpenBankingController {
  ...
  async createConnection(@Request() req, @Body() body: any) {
  ...
  async getUserConnections(@Request() req) {
    ...
    const patched = connections.map((conn) => {
      if (conn.balance === 0 && mockAccounts.length > 0) {
        ...
        const last4 =
          conn.accountNumberMask?.replace(/\*/g, '').slice(-4) || '';
        const mockAcc = mockAccounts.find(
          (a) => a.accountNumber?.slice(-4) === last4,
        );
        if (mockAcc?.balance) {
          console.log(
            `[OpenBanking] Patching balance for ${conn.bankCode} from MOCK: ${mockAcc.balance}`,
          );
          return { ...conn, balance: mockAcc.balance };
        }
      }
      console.log(
        `[OpenBanking] Connection: bankCode=${conn.bankCode}, balance=${conn.balance}`,
      );
      return conn;
    });
