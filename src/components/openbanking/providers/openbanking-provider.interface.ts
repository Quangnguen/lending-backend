...
export interface IOpenBankingProvider {
  ...
  initiateConsent(
    ...
    bankCode: string,
    permissions: ConsentPermission[],
  ): Promise<{ consentId: string; authorizeUrl?: string; otpRequired?: boolean }>;
   ...
   * Lấy số dư tài khoản
   */
  getBalance(consentId: string, accountId: string): Promise<{ balance: number; availableBalance: number; currency: string }>;
  ...
  getTransactions(
    ...
    page?: number,
    limit?: number,
  ): Promise<{ transactions: OBTransaction[]; totalCount: number; hasMore: boolean }>;
