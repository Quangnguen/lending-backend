...
export class CreditService {
  ...
  async calculateCreditScore(userId: string): Promise<CreditScore> {
    ...
    if (!hasRealConnections) {
      this.logger.log(
        `User ${userId} has no bank connections. Return score 0 (UNRATED)`,
      );
    ...
    if (hasRealConnections) {
      ...
      this.logger.log(
        `User ${userId} has ${bankConnections.length} real bank connections`,
      );
      ...
      for (const conn of bankConnections) {
        if (conn.linkedAt) {
          ...
          const monthsLinked =
            (Date.now() - linkedDate.getTime()) / (1000 * 3600 * 24 * 30);
          connectionHistoryScore = Math.max(
            connectionHistoryScore,
            monthsLinked,
          );
        }
      }
      ...
      for (const acc of mockAccounts) {
        ...
        try {
          const transactions =
            await this.mockOpenBankingService.getTransactions(acc.id);
          ...
          this.logger.error(
            `Error fetching transactions for account ${acc.id}: ${error.message}`,
          );
        }
      }
    }
