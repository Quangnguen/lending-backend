...
let ratesCache: { ethUsd: number; usdtVnd: number; updatedAt: number } | null =
  null;
...
async function fetchLiveRates() {
  ...
  try {
    ...
    if (!ratesCache)
      ratesCache = { ethUsd: 2000, usdtVnd: 25000, updatedAt: Date.now() };
  }
  return ratesCache;
}
...
export class CreditController {
  ...
  @ApiOperation({
    summary:
      'Tỷ giá thị trường: ETH/USD và USDT/VND (cache 5 phút từ CoinGecko)',
  })
  async getRates() {
  ...
  async getScore(@Request() req) {
    ...
    return {
      ...score,
      collateralRatio: this.creditScoringEngine.getRequiredCollateralRatio(
        score.score,
      ),
    };
  }
  ...
  @ApiOperation({
    summary:
      'Force recalculate credit score using enhanced engine (Open Banking + Loan History)',
  })
  ...
  @Get('history')
  @ApiOperation({
    summary: 'Get credit score history (track score changes over time)',
  })
  @ApiQuery({
    name: 'limit',
    required: false,
    type: Number,
    description: 'Số lượng records (default: 10)',
  })
  async getScoreHistory(@Request() req, @Query('limit') limit?: string) {
    const userId = req.user.id;
    const scores = await this.creditScoringEngine.getScoreHistory(
      userId,
      Number(limit) || 10,
    );
  ...
  @ApiOperation({
    summary: 'Get required collateral ratio based on current credit score',
  })
  async getCollateralRatio(@Request() req) {
    ...
    const collateralRatio = this.creditScoringEngine.getRequiredCollateralRatio(
      score.score,
    );
    ...
    return {
      ...
      rating: score.rating,
      collateralRatio,
      collateralRatioDescription:
        this.getCollateralDescription(collateralRatio),
  ...
  @ApiOperation({
    summary: '[Legacy] Calculate credit score using original algorithm',
  })
  ...
  private getCollateralDescription(ratio: number): string {
    if (ratio <= 50)
      return 'Tín chấp/Thế chấp rất thấp (EXCELLENT) — Chỉ cần 50% giá trị khoản vay';
    if (ratio <= 80)
      return 'Thế chấp thấp (VERY GOOD) — Cần 80% giá trị khoản vay';
    if (ratio <= 100)
      return 'Thế chấp vừa phải (GOOD) — Cần 100% giá trị khoản vay';
    if (ratio <= 120)
      return 'Thế chấp tiêu chuẩn (FAIR) — Cần 120% giá trị khoản vay';
    if (ratio <= 135)
      return 'Thế chấp cao (BELOW FAIR) — Cần 135% giá trị khoản vay';
  ...
  async getAdminScores() {
    const latestScores =
      await this.creditScoringEngine.getLatestScoresForAllUsers();
  ...
  @ApiOperation({
    summary: '[Admin] Get latest credit score of a specific user',
  })
