...
export class OraclePublisherService implements OnModuleInit {
  ...
  async onModuleInit() {
    try {
      ...
      const oracleContractAddress = this.configService.get<string>(
      ...
      if (
        updaterAddress.toLowerCase() !== this.oracleWallet.address.toLowerCase()
      ) {
        this.logger.error(
  ...
  async publishScore(
    ...
    if (!this.isConfigured) {
      this.logger.debug(
        `[OraclePublisher] Not configured - skipping publish for ${walletAddress}`,
      );
      return null;
    }
    ...
    try {
      ...
      if (!ethers.isAddress(walletAddress)) {
        this.logger.error(
          `[OraclePublisher] Invalid address: ${walletAddress}`,
        );
        return null;
      }
      ...
      this.logger.log(
        `[OraclePublisher] Publishing score ${score} for ${walletAddress}...`,
      );

      const tx = await this.oracleContract.updateCreditScore(
        walletAddress,
        score,
      );
      ...
      this.logger.error(
        `[OraclePublisher] ❌ Failed to publish score: ${error.message}`,
      );
      return null;
    }
  }
  ...
  async publishBatchScores(
    ...
    if (!this.isConfigured) {
      this.logger.debug(
        '[OraclePublisher] Not configured - skipping batch publish',
      );
      return null;
    }
    ...
    try {
      ...
      this.logger.log(
        `[OraclePublisher] Batch publishing ${entries.length} scores...`,
      );
      ...
      this.logger.error(
        `[OraclePublisher] ❌ Batch publish failed: ${error.message}`,
      );
      return null;
    }
  }
  ...
  async getOnChainScore(walletAddress: string): Promise<{
    ...
    try {
      const [score, timestamp, isValid] =
        await this.oracleContract.getCreditScore(walletAddress);
      const ratio =
        await this.oracleContract.getRequiredCollateralRatio(walletAddress);
      ...
      this.logger.error(
        `[OraclePublisher] Failed to read on-chain score: ${error.message}`,
      );
      return null;
    }
  }
