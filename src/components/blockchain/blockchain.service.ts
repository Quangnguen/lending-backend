...
export class BlockchainService implements OnModuleInit {
  ...
  constructor(
    ...
    const rpcUrl =
      configService.get('BLOCKCHAIN_RPC_URL') || 'http://127.0.0.1:7545';
  ...
  async onModuleInit() {
    try {
      ...
      this.logger.log(
        `✅ Kết nối blockchain thành công - ChainID: ${network.chainId}`,
      );
  ...
  private _validatePrivateKey(privateKey: string, rpcUrl: string): void {
    ...
    const keyNormalized = privateKey.startsWith('0x')
      ? privateKey.slice(2)
      : privateKey;
    if (KNOWN_TEST_KEYS.has(keyNormalized.toLowerCase())) {
      const msg =
        '[SECURITY] ORACLE_PRIVATE_KEY là private key test (Ganache). KHÔNG dùng trong production!';
      if (isProduction) {
        throw new Error(msg);
      } else {
  ...
  // ============================
  async getLoanOnChainStatus(
    loanContractAddress: string,
  ): Promise<OnChainLoanStatus | null> {
    try {
      ...
      this.logger.error(
        `Lỗi đọc loan on-chain ${loanContractAddress}: ${error.message}`,
      );
      return null;
    }
  }
  ...
  async verifyTransaction(
    txHash: string,
  ): Promise<TransactionVerification | null> {
    try {
  ...
  async startEventListener() {
    ...
    try {
      ...
      this.p2pLendingContract.on(
        'LoanRequestCreated',
        async (requestId, borrower, principal) => {
          this.logger.log(
            `📝 [Event] LoanRequestCreated — ID: ${requestId}, Borrower: ${borrower}, Principal: ${ethers.formatUnits(principal, 6)} USDT`,
          );
      ...
      this.p2pLendingContract.on(
        ...
        async (requestId, borrower) => {
          this.logger.log(
            `❌ [Event] LoanRequestCancelled — ID: ${requestId}, Borrower: ${borrower}`,
          );
        },
      );
      ...
      this.p2pLendingContract.on(
        'LoanMatched',
        async (requestId, lender, loanContractAddr) => {
          this.logger.log(
            `💰 [Event] LoanMatched — RequestID: ${requestId}, Lender: ${lender}, LoanContract: ${loanContractAddr}`,
          );
          await this.handleLoanFundedEvent(
            Number(requestId),
            lender,
            loanContractAddr,
          );
  ...
  private async attachLoanContractListeners() {
    try {
      ...
      this.logger.log(
        `Attached listeners cho ${activeLoans.length} active loans`,
      );
    } catch (error) {
  ...
  attachSingleLoanListener(loanContractAddress: string) {
    try {
      ...
      loanContract.on('LoanRepaid', async (loanId, totalAmount) => {
        this.logger.log(
          `✅ [Event] LoanRepaid — ID: ${loanId}, Amount: ${ethers.formatUnits(totalAmount, 6)} USDT`,
        );
        await this.handleLoanRepaidEvent(
          loanContractAddress,
          Number(totalAmount),
        );
      });
      ...
      loanContract.on('LoanLiquidated', async (loanId, liquidator) => {
        this.logger.log(
          `⚠️ [Event] LoanLiquidated — ID: ${loanId}, Liquidator: ${liquidator}`,
        );
      ...
      loanContract.on(
        'CollateralReleaseFailed',
        async (loanId, borrower, amount, reason) => {
          this.logger.error(
          ...
          try {
            ...
            this.logger.error(
              `Không thể flag collateralStuck cho ${loanContractAddress}: ${e.message}`,
            );
          }
        },
      );
      ...
      this.logger.error(
        `Lỗi attach listener cho ${loanContractAddress}: ${error.message}`,
      );
    }
  }
  ...
  private async handleLoanFundedEvent(
    requestId: number,
    lender: string,
    loanContractAddr: string,
  ) {
    try {
      // onChainRequestId nằm trong LoanRequest, không phải Loan → cần tìm qua 2 bước
      const loanRequest = await this.loanRequestModel.findOne({
        onChainRequestId: requestId,
      });
      if (!loanRequest) {
        this.logger.log(
          `LoanRequest với onChainRequestId=${requestId} chưa có trong DB`,
        );
        return;
      }
      ...
      if (!loan) {
        this.logger.log(
          `Loan cho requestId=${loanRequest._id} chưa có trong DB`,
        );
        return;
      }
      ...
      await loan.save();
      this.logger.log(
        `✅ Đã set loanContractAddress=${loanContractAddr} cho loan ${loan._id}`,
      );
  ...
  async getPendingRequests(): Promise<any[]> {
    try {
      ...
      const pendingIds: bigint[] =
        await this.p2pLendingContract.getPendingRequests();
      const requests = [];
  ...
  async syncLoanStatus(loanId: string): Promise<boolean> {
    try {
      ...
      if (!loan || !loan.loanContractAddress) {
        this.logger.warn(
          `Loan ${loanId} không tồn tại hoặc chưa có contract address`,
        );
        return false;
      }

      const onChainStatus = await this.getLoanOnChainStatus(
      ...
      if (mappedStatus && loan.status !== mappedStatus) {
        ...
        await loan.save();
        this.logger.log(
          `🔄 Sync loan ${loanId}: ${oldStatus} → ${loan.status}`,
        );
      }
  ...
  async checkOverdueLoans(): Promise<number> {
    ...
    try {
      ...
      for (const loan of overdueLoans) {
        ...
        await loan.save();
        this.logger.warn(
          `🔴 Loan ${loan._id} chuyển sang DEFAULTED off-chain.`,
        );
      }
  ...
  async isRequestActive(onChainRequestId: number): Promise<boolean> {
    try {
      ...
      const active =
        await this.p2pLendingContract.requestActive(onChainRequestId);
      return Boolean(active);
    } catch (error) {
      this.logger.warn(
        `isRequestActive check failed (requestId=${onChainRequestId}): ${error.message} — allowing through`,
      );
