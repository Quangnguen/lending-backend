...
export class LiquidationService {
  ...
  private async initBlockchain() {
    try {
      ...
      const debtTokenAddress =
        this.configService.get<string>('DEBT_TOKEN_ADDRESS');

      if (!rpcUrl || !privateKey) {
        this.logger.warn(
          '[Liquidation] Missing blockchain config - running in DB-only mode',
        );
        return;
      }
      ...
      if (debtTokenAddress) {
        ...
        this.logger.log(
          `[Liquidation] ✅ Configured | DebtToken: ${debtTokenAddress}`,
        );
      } else {
        this.logger.warn(
          '[Liquidation] No DEBT_TOKEN_ADDRESS - DebtToken minting disabled',
        );
      }
    } catch (error) {
  ...
  private readonly PRICE_LIQUIDATION_THRESHOLD = 1.1;
  ...
  async scanPriceBasedLiquidations(): Promise<void> {
    ...
    for (const loan of activeLoans) {
      try {
        ...
        this.logger.error(
          `[PriceBot] Failed for loan ${loan._id}: ${err.message}`,
        );
      }
    }
    ...
    if (liquidated > 0) {
      this.logger.warn(
        `[PriceBot] ✅ Liquidated ${liquidated} loans at ETH=$${ethPrice}`,
      );
    }
  }
  ...
  async scanOverdueLoans(): Promise<void> {
    ...
    const longOverdueLoans = await this.loanModel
      .find({
        status: LOAN_STATUS_ENUM.OVERDUE,
        dueDate: {
          $lt: new Date(now.getTime() - this.DEFAULT_DAYS * 24 * 3600 * 1000),
        },
      })
      .lean();

    // 3. DEFAULTED → LIQUIDATED: quá dueDate + 60 ngày
    const toLiquidateLoans = await this.loanModel
      .find({
        status: LOAN_STATUS_ENUM.DEFAULTED,
        dueDate: {
          $lt: new Date(
            now.getTime() - this.LIQUIDATION_DAYS * 24 * 3600 * 1000,
          ),
        },
      })
      .lean();
  ...
  private async processOverdue(loan: any) {
    try {
      ...
      this.notificationService
        ...
        .catch((e) =>
          this.logger.warn(`[Liquidation] Notify OVERDUE failed: ${e.message}`),
        );
      ...
      this.logger.error(
        `[Liquidation] Error processing overdue ${loan._id}: ${error.message}`,
      );
    }
  }
  ...
  private async processDefault(loan: any) {
    try {
      ...
      this.notificationService
        ...
        .catch((e) =>
          this.logger.warn(
            `[Liquidation] Notify DEFAULTED failed: ${e.message}`,
          ),
        );
      ...
      this.logger.error(
        `[Liquidation] Error processing default ${loan._id}: ${error.message}`,
      );
    }
  }
  ...
  private async processLiquidation(loan: any) {
    try {
      ...
      const transactionHash = await this.triggerOnChainLiquidation(loan);

      await this.notificationService
        .notifyLoanLiquidated({
          borrowerId,
        ...
        .catch((e) =>
          this.logger.warn(
            `[Liquidation] Notify LIQUIDATED failed: ${e.message}`,
          ),
        );
      ...
      this.logger.error(
        `[Liquidation] Error processing liquidation ${loan._id}: ${error.message}`,
      );
    }
  }
  ...
  async mintDebtTokenOnChain(loan: any): Promise<string | null> {
    ...
    try {
      ...
      const borrower = await this.userModel
        .findById(loan.borrowerId)
        .select('walletAddress')
        .lean();

      if (!borrower || !(borrower as any).walletAddress) {
        this.logger.warn(
          `[Liquidation] Borrower ${loan.borrowerId} has no walletAddress`,
        );
        return null;
      }
      ...
      this.logger.log(
        `[Liquidation] Minting DebtToken for ${walletAddress}...`,
      );
      ...
      this.logger.error(
        `[Liquidation] DebtToken mint failed: ${error.message}`,
      );
      return null;
    }
  }
  ...
  private async triggerOnChainLiquidation(loan: any): Promise<string | null> {
    ...
    try {
      ...
      if (!isOverdue) {
        this.logger.debug(
          `[Liquidation] Loan ${loan.loanContractAddress} not overdue on-chain yet`,
        );
        return null;
      }
      ...
      this.logger.error(
        `[Liquidation] On-chain liquidation failed: ${error.message}`,
      );
      return null;
    }
  }
    ...
    return {
      overdue: overdue.map((l) => ({
        ...
        daysOverdue: Math.ceil(
          (now.getTime() - l.dueDate.getTime()) / (24 * 3600 * 1000),
        ),
        daysUntilDefault: Math.max(
          0,
          this.DEFAULT_DAYS -
            Math.ceil(
              (now.getTime() - l.dueDate.getTime()) / (24 * 3600 * 1000),
      ...
      summary: {
        ...
        totalAtRisk: overdue.reduce(
          (s, l) => s + (l.remainingAmount || l.totalAmount),
          0,
        ),
        totalDefaulted: defaulted.reduce(
          (s, l) => s + (l.remainingAmount || l.totalAmount),
          0,
        ),
      },
  ...
  async checkBorrowerDebtTokens(walletAddress: string) {
    if (!this.isConfigured || !this.debtTokenContract) {
      return {
        configured: false,
        message: 'DebtToken contract not configured',
      };
    }
    ...
    try {
      ...
      const debtCount =
        await this.debtTokenContract.getDebtCount(walletAddress);
      const totalDebt =
        await this.debtTokenContract.getTotalDebt(walletAddress);
  ...
  generateRepaymentSchedule(
    ...
    const totalInterest =
      (principalAmount * interestRate * durationDays) / (365 * 100);
    const totalAmount = principalAmount + totalInterest;
    const principalPerInstallment =
      Math.round((principalAmount / installments) * 100) / 100;
    const interestPerInstallment =
      Math.round((totalInterest / installments) * 100) / 100;
    ...
    for (let i = 1; i <= installments; i++) {
      const dueDate = new Date(
        startDate.getTime() +
          (durationDays / installments) * i * 24 * 3600 * 1000,
      );
      ...
      const thisInterest = isLast
        ? Math.round(
            (totalInterest - interestPerInstallment * (installments - 1)) * 100,
          ) / 100
