...
export class LoanController {
  ...
  @ApiOperation({
    summary: 'Danh sách yêu cầu vay đang chờ (marketplace cơ bản)',
  })
  ...
  @ApiOperation({
    summary:
      'Marketplace nâng cao — Loan requests kèm risk metrics & credit score',
  })
  @ApiQuery({ name: 'minAmount', required: false, type: Number })
  @ApiQuery({ name: 'maxAmount', required: false, type: Number })
  @ApiQuery({ name: 'minRate', required: false, type: Number })
  ...
  @ApiQuery({
    name: 'sortBy',
    required: false,
    enum: ['rate', 'amount', 'risk', 'return'],
  })
  @ApiQuery({ name: 'sortOrder', required: false, enum: ['asc', 'desc'] })
  async exploreMarketplace(@Request() req, @Query() filters) {
    return this.marketplaceService.getEnrichedMarketplace(
      req.user._id,
      filters,
    );
  }
  ...
  async getRecommendations(@Request() req, @Query('limit') limit?: string) {
    return this.marketplaceService.getRecommendedLoans(
      req.user._id,
      Number(limit) || 5,
    );
  }
  ...
  @ApiOperation({
    summary: 'Risk assessment chi tiết cho 1 loan request (trước khi fund)',
  })
  ...
  @Get(':id/disbursement')
  @ApiOperation({
    summary: 'Hướng dẫn giải ngân — Off-ramp từ crypto về ngân hàng',
  })
  async getDisbursementInstructions(@Request() req, @Param('id') id: string) {
    return this.disbursementService.createDisbursementInstructions(
      req.user._id,
      id,
    );
  }
  ...
  @ApiOperation({
    summary: 'Trạng thái giải ngân tổng hợp (on-chain + off-ramp)',
  })
  ...
  @Roles(ROLE_ENUM.ADMIN, ROLE_ENUM.SUPER_ADMIN)
  @ApiOperation({
    summary: '[Admin] Danh sách yêu cầu vay của một user cụ thể',
  })
  ...
  @Roles(ROLE_ENUM.ADMIN, ROLE_ENUM.SUPER_ADMIN)
  @ApiOperation({
    summary: '[TEST] Trigger thủ công cron quét quá hạn / thanh lý',
  })
  ...
  async setLoanDueForTest(
    ...
    return this.loanService.setLoanDueForTest(
      loanId,
      body.daysOffset,
      body.status,
    );
  ...
  @Roles(ROLE_ENUM.ADMIN, ROLE_ENUM.SUPER_ADMIN)
  @ApiOperation({
    summary: '[TEST] Mint DebtToken trực tiếp cho loan (bỏ qua kiểm tra ngày)',
  })
  ...
  @ApiOperation({
    summary: 'Kiểm tra DebtToken (nợ xấu on-chain) của borrower',
  })
  ...
  @ApiQuery({
    ...
    required: false,
    type: Number,
    description: 'Số kỳ (0 = tự tính theo tháng)',
  })
  ...
  async syncLoanStatus(@Param('loanId') loanId: string) {
    ...
    return {
      success: result,
      message: result ? 'Đồng bộ thành công' : 'Không thể đồng bộ',
    };
  }
