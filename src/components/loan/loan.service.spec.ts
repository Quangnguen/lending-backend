...
describe('LoanService', () => {
  ...
  beforeEach(async () => {
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        LoanService,
        {
          provide: getModelToken(LoanRequest.name),
          useValue: mockLoanRequestModel,
        },
        { provide: getModelToken(Loan.name), useValue: mockLoanModel },
        {
          provide: getModelToken(LoanRepayment.name),
          useValue: mockRepaymentModel,
        },
        ...
        {
          provide: MockOpenBankingService,
          useValue: mockMockOpenBankingService,
        },
  ...
  describe('fundLoan', () => {
    ...
    it('should fund successfully when valid', async () => {
      ...
      mockBlockchainService.verifyTransaction.mockResolvedValue({
        isValid: true,
      });
      ...
      expect(
        mockBlockchainService.attachSingleLoanListener,
  ...
  describe('repayLoan', () => {
    ...
    it('should repay successfully when valid', async () => {
      ...
      mockRepaymentModel.create.mockResolvedValue({
        _id: new Types.ObjectId(),
      });
  ...
  describe('cancelLoanRequest', () => {
    ...
    it('should reject if not the owner', async () => {
      ...
      await expect(service.cancelLoanRequest(userId, 'req-id')).rejects.toThrow(
        ForbiddenException,
      );
    });
