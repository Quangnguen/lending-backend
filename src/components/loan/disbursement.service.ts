  ...
  async createDisbursementInstructions(borrowerId: string, loanId: string) {
    ...
    if (loan.borrowerId.toString() !== borrowerId.toString()) {
      throw new BadRequestException(
        'Bạn không phải borrower của khoản vay này',
      );
    }
    if (loan.status !== LOAN_STATUS_ENUM.ACTIVE) {
      throw new BadRequestException(
        `Khoản vay chưa được giải ngân (status: ${loan.status})`,
      );
    }

    // 2. Lấy bank connection của borrower
    const borrowerBanks =
      await this.openBankingService.getUserConnections(borrowerId);
    ...
    if (!borrowerBanks || borrowerBanks.length === 0) {
      return {
        ...
        instructions: {
          ...
          step2:
            'Để rút VND, hãy liên kết tài khoản ngân hàng qua Open Banking',
          step3:
            'Sau đó, bạn có thể đổi USDT → VND qua sàn giao dịch và rút về NH',
        },
        onChainInfo: {
