...
export class LenderMarketplaceService {
  ...
  async getEnrichedMarketplace(
    ...
    const requests = await this.loanRequestModel
      ...
      .populate(
        'borrowerId',
        'fullName email avatarUrl walletAddress kycStatus createdAt',
      )
      .sort({ createdAt: -1 })
      .lean();
    ...
    if (filters?.riskLevel) {
      filtered = enrichedRequests.filter(
        (r) => r.riskMetrics.riskLevel === filters.riskLevel,
      );
    }
    ...
    return {
      ...
      portfolio,
      riskDistribution: {
        low: enrichedRequests.filter((r) => r.riskMetrics.riskLevel === 'LOW')
          .length,
        medium: enrichedRequests.filter(
          (r) => r.riskMetrics.riskLevel === 'MEDIUM',
        ).length,
        high: enrichedRequests.filter((r) => r.riskMetrics.riskLevel === 'HIGH')
          .length,
      },
    };
  }
  ...
  async getLoanRequestRiskAssessment(requestId: string) {
    ...
    const borrowerHistory = await this.getBorrowerLoanHistory(
      request.borrowerId._id?.toString() || request.borrowerId.toString(),
    );
  ...
  async getRecommendedLoans(lenderId: string, limit = 5) {
    ...
    const pastInvestments = await this.loanModel
      .find({
    ...
    const avgRate =
      pastInvestments.length > 0
        ? pastInvestments.reduce((s, l) => s + l.interestRate, 0) /
          pastInvestments.length
        : 10; // Default 10%
    const avgAmount =
      pastInvestments.length > 0
        ? pastInvestments.reduce((s, l) => s + l.principalAmount, 0) /
          pastInvestments.length
  ...
  async getLenderPortfolioSummary(lenderId: string) {
    const investments = await this.loanModel
      .find({
        lenderId: new Types.ObjectId(lenderId),
      })
      .lean();
    ...
    const active = investments.filter(
      (l) => l.status === LOAN_STATUS_ENUM.ACTIVE,
    );
    const repaid = investments.filter(
      (l) => l.status === LOAN_STATUS_ENUM.REPAID,
    );
    const defaulted = investments.filter(
      (l) =>
        l.status === LOAN_STATUS_ENUM.DEFAULTED ||
        l.status === LOAN_STATUS_ENUM.LIQUIDATED,
    );
    ...
    const totalReturn = repaid.reduce(
      (s, l) => s + (l.amountPaid || 0) - l.principalAmount,
      0,
    );
    const avgRate =
      investments.reduce((s, l) => s + l.interestRate, 0) / investments.length;

    // Diversification: số borrowers khác nhau / tổng investments
    const uniqueBorrowers = new Set(
      investments.map((l) => l.borrowerId.toString()),
    ).size;
    const diversificationScore = Math.min(
      100,
      Math.round((uniqueBorrowers / investments.length) * 100),
    );
  ...
  private async enrichLoanRequest(request: any) {
    ...
    const collateralRatio =
      this.creditScoringEngine.getRequiredCollateralRatio(score);
  ...
  private calculateRiskMetrics(
    ...
    if (creditScore >= 800)
      ...
      defaultProbability = 0.05; // 5%
    else if (creditScore >= 600)
      defaultProbability = 0.08; // 8%
    else if (creditScore >= 500)
      defaultProbability = 0.15; // 15%
    else if (creditScore >= 400)
      defaultProbability = 0.25; // 25%
    else if (creditScore > 0)
      defaultProbability = 0.4; // 40%
    else defaultProbability = 0.5; // 50% (no score)

    // Adjust theo collateral ratio (thế chấp cao = risk thấp hơn)
    if (collateralRatio >= 150)
      defaultProbability *= 0.5; // Half risk khi full collateral
    ...
    const interestAmount =
      ...
      (365 * 100);
    const expectedLoss =
      request.loanAmount * defaultProbability * (1 - collateralRatio / 100);
    const riskAdjustedReturn =
      Math.round((interestAmount - Math.max(0, expectedLoss)) * 100) / 100;
  ...
  private async getBorrowerLoanHistory(borrowerId: string) {
    ...
    const overdue = loans.filter(
      (l) => l.status === LOAN_STATUS_ENUM.OVERDUE,
    ).length;
  ...
  private generateRecommendation(
    ...
    if (risk.riskLevel === 'LOW' && credit.creditScore >= 600) {
      ...
      if (borrowerHistory.repaidOnTime > 0) {
        reasons.push(
          `Đã trả đúng hạn ${borrowerHistory.repaidOnTime} khoản vay`,
        );
      }
    ...
    if (risk.riskLevel === 'MEDIUM') {
      reasons.push(
        `Mức rủi ro trung bình (${risk.defaultProbability}% default)`,
      );
      if (borrowerHistory.defaultedLoans > 0) {
        reasons.push(
          `Cảnh báo: Đã có ${borrowerHistory.defaultedLoans} lần vỡ nợ`,
        );
      }
    ...
    if (borrowerHistory.isNewBorrower)
      reasons.push('Người vay mới, chưa có lịch sử');
    if (borrowerHistory.overdueLoans > 0)
      reasons.push(`Đang có ${borrowerHistory.overdueLoans} khoản vay quá hạn`);
  ...
  private sortRequests(requests: any[], sortBy?: string, sortOrder?: string) {
    ...
    return requests.sort((a, b) => {
      switch (sortBy) {
        case 'rate':
          return (a.interestRate - b.interestRate) * order;
        case 'amount':
          return (a.loanAmount - b.loanAmount) * order;
        case 'risk':
          return (a.riskMetrics.riskScore - b.riskMetrics.riskScore) * order;
        case 'return':
          return (
            (a.expectedReturn.interestAmount -
              b.expectedReturn.interestAmount) *
            order
          );
        default:
          return 0;
